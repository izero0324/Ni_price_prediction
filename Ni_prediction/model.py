...
class decoder(nn.Module):
    ...
    def forward(self, input_encoded, y_history):
        ...
        for t in range(self.T-1):
            ...
            x = F.softmax(self.attn_layer(x.view(-1, 2 * self.decoder_hidden_size + self.encoder_hidden_size)).view(-1, self.T-1)) # batch_size * T - 1, row sum up to 1
        ...
        # Eqn. 22: final output
        y_pred = self.fc_final(torch.cat((hidden[0], context), dim = 1)).squeeze(1)
...
# Train the model
class da_rnn:
    def __init__(self, file_data, logger, encoder_hidden_size = 64, decoder_hidden_size = 64, T = 30,
        ...
        self.X = dat.loc[:, [x for x in dat.columns.tolist() if x != 'LME_Nickel_close']].as_matrix()
        self.y = np.array(dat.LME_Nickel_close)
...
model = da_rnn(file_data = 'C://Users/Izero0324/Desktop/comp/Result.csv', logger = logger, parallel = False,
              learning_rate = .001)
